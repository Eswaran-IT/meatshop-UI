...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (mobile: string, password: string): Promise<boolean> => {
    // Demo login logic
    const isCustomer = mobile === DEMO_USERS.CUSTOMER.mobile && password === DEMO_USERS.CUSTOMER.password;
    const isAdmin = mobile === DEMO_USERS.ADMIN.mobile && password === DEMO_USERS.ADMIN.password;

    if (isCustomer || isAdmin) {
      const userData: User = {
        id: Date.now().toString(),
        mobile,
        name: isAdmin ? DEMO_USERS.ADMIN.name : DEMO_USERS.CUSTOMER.name,
        isAdmin: isAdmin
      };
