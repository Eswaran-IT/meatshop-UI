...
const LoyaltyRewards: React.FC = () => {
  ...
  const handleApplyFilters = () => {
    ...
    if (orderCount) {
      ...
      if (match) {
        ...
        filtered = filtered.filter(order => {
          // In real app, count by customerId
    ...
    if (orderDate) {
      // e.g. '2024-03-01' or 'last 30 days'
      if (orderDate.startsWith('last')) {
        const days = parseInt(orderDate.replace(/\D/g, ''));
        const since = new Date();
        since.setDate(since.getDate() - days);
        filtered = filtered.filter(order => new Date(order.orderDate) >= since);
      } else {
        filtered = filtered.filter(order => order.orderDate.startsWith(orderDate));
      }
    }
    if (amount) {
    ...
    if (productType) {
      filtered = filtered.filter(order => order.items.some(i => i.meatName.toLowerCase().includes(productType.toLowerCase())));
    }
    ...
    <div className="max-w-6xl mx-auto py-8">
      <Card>
        ...
        <CardContent>
          ...
          {/* Filtering UI (all options, does not affect table, only offer creation) */}
          <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
            ...
            <Input placeholder="Order Count (e.g. >2)" className="w-full" value={orderCount} onChange={e => setOrderCount(e.target.value)} />
            <Input placeholder="Order Dates (e.g. last 30 days)" className="w-full" value={orderDate} onChange={e => setOrderDate(e.target.value)} />
            <Input placeholder="Total Purchase Amount (e.g. >1000)" className="w-full" value={amount} onChange={e => setAmount(e.target.value)} />
            <Input placeholder="Product Type (e.g. Chicken)" className="w-full" value={productType} onChange={e => setProductType(e.target.value)} />
