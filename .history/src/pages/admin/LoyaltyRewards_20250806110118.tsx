  ...
  const handleApplyFilters = () => {
    ...
    if (search) {
      ...
      filtered = filtered.filter(order =>
        ...
        order.customerMobile.includes(s)
      );
    }
    if (category) {
      filtered = filtered.filter(order =>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
            <Input placeholder="Filter by Customer Name or Number" className="w-full" value={search} onChange={e => setSearch(e.target.value)} />
            <Input placeholder="Category/Product Type (e.g. Chicken)" className="w-full" value={category} onChange={e => setCategory(e.target.value)} />
