...
const HomePage: React.FC = () => {
  ...
  // Always show 3 products per category, filtered by search
  let filteredProducts: typeof featuredProducts = [];
  categories.forEach(cat => {
    const products = featuredProducts.filter(p => {
      const matchesCategory = activeCategory === null || cat.id === activeCategory;
      const matchesSearch = p.name.toLowerCase().includes(searchTerm.toLowerCase());
      return p.category === cat.name && matchesCategory && matchesSearch;
    }).slice(0, 3);
