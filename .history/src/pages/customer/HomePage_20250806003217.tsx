...
const HomePage: React.FC = () => {
      ...
      toast({
        title: (
          <span className="flex items-center gap-2 text-green-600">
            <span className="inline-block"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></span>
            Added to Cart
          </span>
        ),
  ...
  // Always show 3 products per category, and when searching, ignore category filter and show up to 3 matches per category
  let filteredProducts: typeof featuredProducts = [];
  if (searchTerm.trim() === "") {
    categories.forEach(cat => {
      const products = featuredProducts.filter(p => p.category === cat.name).slice(0, 3);
